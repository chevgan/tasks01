function createScreen(width, height) {
  ...
  function rotateRow(screen, row, by) {
    const width = screen[row].length;
    const newRow = Array(width).fill('.');
    
    for (let x = 0; x < width; x++) {
      newRow[(x + by) % width] = screen[row][x];
    }
  ...
  function rotateColumn(screen, col, by) {
    const height = screen.length;
    const newColumn = Array(height).fill('.');
  
    for (let y = 0; y < height; y++) {
      newColumn[(y + by) % height] = screen[y][col];
    }
  
    for (let y = 0; y < height; y++) {
      screen[y][col] = newColumn[y];
    }
  }
  ...
  function processInstructions(instructions) {
    ...
    for (const instruction of instructions) {
      ...
      if (parts[0] === 'rect') {
        ...
        const index = Number(parts[2].split('=')[1]);
        const by = Number(parts[4]);
        
        if (parts[1] === 'row') {
          rotateRow(screen, index, by);
        } else if (parts[1] === 'column') {
          rotateColumn(screen, index, by);
        }
      }
    }
